...
class VisualizeNode(object):
    ...
    with tf.Graph().as_default():
      ...
      with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for file in npyFiles:
          lidarPrediction = np.load(file).astype(np.float32, copy=False)
          ...
          new_layer = np.zeros_like(lidarPrediction[:, :, 0])
          lidarPrediction = np.concatenate((lidarPrediction, new_layer[:, :, np.newaxis]), axis=2)
          print("lidarPrediction", lidarPrediction.shape)
  ...
  def npyPublish(self, npyFile, file):
    ...
    # label3D[np.where(label==0)] = [1., 0., 0.] #uncknown
    # label3D[np.where(label==1)] = [1., 1., 0.] #car
    # label3D[np.where(label==2)] = [0., 1., 0.] #pedestrian
    # label3D[np.where(label==3)] = [0., 1., 1.] #cyclist
    # label3D[np.where(label==4)] = [0., 0., 1.] #ground

    label3D[np.where(label==0)] = [1., 1., 1.] #uncknown
    label3D[np.where(label==1)] = [1., 1., 1.] #car
    label3D[np.where(label==2)] = [1., 1., 1.] #pedestrian
    label3D[np.where(label==3)] = [1., 1., 1.] #cyclist
    label3D[np.where(label==4)] = [0., 0., 1.] #ground
    ...
    #YO
    path = "/home/iiarroyo/Documents/bags_estancia/pasillo_central_persona"
